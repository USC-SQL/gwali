...
import edu.usc.util.XPathDistanceMeasurer;
...
public class DomUtils {
	...
	// updated by Abdulmajeed Alameer (alameer@usc.edu) so Lavenstien distance is computed based on xpath elements basis instead of character basis
	...
	public static float calculateMatchIndex(DomNode x, DomNode y) {
		...
		XPathDistanceMeasurer xpathDistMsr = new XPathDistanceMeasurer(xpaths);
		
		float xPathSim = 1 - xpathDistMsr.computeDistance(xPath1, xPath2, true);
				//StringUtils.getLevenshteinDistance(xPath1, xPath2) / (float) Math.max(xPath1.length(), xPath2.length());
